...
class http_conn{
    public:
        ...
        //HTTP请求方法，但我们只支持GET
        enum METHOD {GET = 0, POST, HEAR, PUT, DELETE, TRACE, OPTIONS, CONNECT};
        /*
            解析客户端请求时，主状态机的状态
        ...
        */    
        ...
        //从状态机的三种可能状态，即行的读取状态：1.读取到一个完整的行 2.行出错 3. 行数据尚且不完全
    ...
    private:
        ...
        int m_read_idx = 0;//标识读缓冲区中以及读入的客户端数据的最后一个字节的下标
        
        int m_checked_idx = 0; //当前正在分析的字符在读缓冲区的位置
        int m_start_line = 0;//正在解析的行的起始位置
